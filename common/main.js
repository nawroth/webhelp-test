 ...
 * http://kasunbg.blogspot.com David Cramer, http://www.thingbag.net
 * 
 */
...
/**
 * If an user moved to another page by clicking on a toc link, and then clicked
 * on #searchDiv, search should be performed if the cookie textToSearch is not
 * empty.
 */
function doSearch() {
 ...
 * document.getElementById("tree"); //get table of contents Div var divs =
 * tocDiv.getElementsByTagName("div"); var matchedDivNumber; var i; for (i = 0;
 * i < divs.length; i++) { //1101001 var div = divs[i]; var liNode =
 * div.parentNode; } //create a new cookie if a treeview does not exist if
 * ($.cookie(treeCookieId) == null || $.cookie(treeCookieId) == "") { var
 * branches = $("#tree").find("li");//.prepareBranches(treesettings); var data =
 * []; branches.each(function(i, e) { data[i] = $(e).is(":has(>ul:visible)") ? 1 :
 * 0; }); $.cookie(treeCookieId, data.join("")); }
 * 
 * if (i < divs.length) { var treeviewCookie = $.cookie(treeCookieId); var
 * tvCookie1 = treeviewCookie.substring(0, i); var tvCookie2 =
 * treeviewCookie.substring(i + 1); var newTVCookie = tvCookie1 + "1" +
 * tvCookie2; $.cookie(treeCookieId, newTVCookie); } }
 */
